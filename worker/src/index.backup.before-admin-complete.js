...
async function zarin(env,endpoint,payload){const base=env.PAYMENT_ENV==='production'?'https://payment.zarinpal.com/pg/v4/payment':'https://api.zarinpal.com/pg/v4/payment';const r=await fetch(base+'/'+endpoint,{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify({...payload,merchant_id:env.ZARINPAL_MERCHANT_ID})});return r.json()}
async function route(req,env){const u=new URL(req.url);if(req.method==='OPTIONS')return new Response(null,{status:204,headers:cors(req,env)});
 ...
 if(u.pathname.startsWith('/api/orders/')&&u.pathname.endsWith('/pay')&&req.method==='POST'){if(!me||!requireCsrf(req))return json({error:'unauthorized'},401);const oid=u.pathname.split('/')[3],o=await env.DB.prepare('SELECT * FROM orders WHERE id=? AND user_id=? AND status=\'PENDING\'').bind(oid,me.id).first();if(!o)return json({error:'order_not_payable'},400);let p=await env.DB.prepare('SELECT * FROM payments WHERE order_id=?').bind(oid).first();if(!p)return json({error:'payment_missing'},500);if(!env.ZARINPAL_MERCHANT_ID)return json({error:'payment_not_configured'},503);const reqz=await zarin(env,'request.json',{amount:o.total_irt,description:`GilasArt Order ${oid}`,callback_url:env.PAYMENT_CALLBACK_URL||`${origin(env)}/api/payment/callback`,metadata:{mobile:me.mobile}});const data=reqz.data||{};const authority=data.authority;if(!authority)return json({error:'payment_request_failed',details:reqz.errors||[]},502);await env.DB.batch([env.DB.prepare("UPDATE payments SET status='REDIRECTED',authority=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(authority,p.id),env.DB.prepare('INSERT INTO payment_attempts(id,payment_id,authority,request_code,raw_status) VALUES(?,?,?,?,?)').bind(uid(),p.id,authority,reqz.code||null,JSON.stringify(reqz))]);const host=env.PAYMENT_ENV==='production'?'https://www.zarinpal.com':'https://sandbox.zarinpal.com';return json({ok:true,url:`${host}/pg/StartPay/${authority}`})}
 if(u.pathname==='/api/payment/callback'&&req.method==='GET'){const authority=u.searchParams.get('Authority'),status=u.searchParams.get('Status');if(!authority)return json({error:'missing_authority'},400);const p=await env.DB.prepare('SELECT p.*,o.status order_status FROM payments p JOIN orders o ON o.id=p.order_id WHERE p.authority=?').bind(authority).first();if(!p)return json({error:'payment_not_found'},404);if(status!=='OK') {await env.DB.prepare("UPDATE payments SET status='FAILED',updated_at=CURRENT_TIMESTAMP WHERE id=? AND status!='PAID'").bind(p.id).run();return Response.redirect(`${frontend(env)}/#/payment/failed?order=${p.order_id}`,302)}if(p.status==='PAID')return Response.redirect(`${frontend(env)}/#/payment/success?order=${p.order_id}`,302);if(!env.ZARINPAL_MERCHANT_ID)return json({error:'payment_not_configured'},503);const vr=await zarin(env,'verify.json',{amount:p.amount_irt,authority});const code=vr.code||vr.data?.code,ref=vr.data?.ref_id;const ok=code===100||code===101;await env.DB.batch([env.DB.prepare('INSERT INTO payment_attempts(id,payment_id,authority,verify_code,callback_status,raw_status) VALUES(?,?,?,?,?,?)').bind(uid(),p.id,authority,code||null,status,JSON.stringify(vr)),env.DB.prepare(`UPDATE payments SET status=?,ref_id=COALESCE(?,ref_id),paid_at=CASE WHEN ? THEN CURRENT_TIMESTAMP ELSE paid_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(ok?'PAID':'FAILED',ref,ok,p.id)]);if(ok){await env.DB.batch([env.DB.prepare("UPDATE orders SET status='PAID',updated_at=CURRENT_TIMESTAMP WHERE id=? AND status='PENDING'").bind(p.order_id),...(await env.DB.prepare('SELECT product_id,quantity FROM order_items WHERE order_id=?').bind(p.order_id).all()).results.map(x=>env.DB.prepare('UPDATE inventory SET quantity=quantity-?,updated_at=CURRENT_TIMESTAMP WHERE product_id=? AND quantity>=?').bind(x.quantity,x.product_id,x.quantity))]);return Response.redirect(`${frontend(env)}/#/payment/success?order=${p.order_id}&ref=${encodeURIComponent(ref||'')}`,302)}return Response.redirect(`${frontend(env)}/#/payment/failed?order=${p.order_id}`,302)}
 ...
 if(u.pathname==='/api/admin/products'&&req.method==='GET'){if(!(await roles(me,env)).includes('admin'))return json({error:'forbidden'},403);const r=await env.DB.prepare('SELECT p.*,i.quantity stock FROM products p LEFT JOIN inventory i ON i.product_id=p.id ORDER BY p.created_at DESC').all();return json({items:r.results||[]})}
 if(u.pathname==='/api/admin/products'&&req.method==='POST'){if(!(await roles(me,env)).includes('admin')||!requireCsrf(req))return json({error:'forbidden'},403);const b=await body(req);const id=uid();await env.DB.batch([env.DB.prepare('INSERT INTO products(id,category_id,slug,sku,name,description,price_irt,active,seo_title,seo_description) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(id,b.categoryId||null,b.slug,b.sku,b.name,b.description||'',Number(b.priceIrt)||0,b.active===false?0:1,b.seoTitle||b.name,b.seoDescription||''),env.DB.prepare('INSERT INTO inventory(product_id,quantity) VALUES(?,?)').bind(id,Math.max(0,Number(b.stock)||0))]);await audit(env,me,'admin.product.create','product',id,{sku:b.sku},req);return json({ok:true,id})}
 if(u.pathname==='/api/admin/orders'&&req.method==='GET'){if(!(await roles(me,env)).includes('admin'))return json({error:'forbidden'},403);const r=await env.DB.prepare('SELECT o.*,u.mobile FROM orders o JOIN users u ON u.id=o.user_id ORDER BY o.created_at DESC LIMIT 200').all();return json({items:r.results||[]})}
 if(u.pathname==='/api/admin/stats'&&req.method==='GET'){const rr=await roles(me,env);if(!rr.includes('admin'))return json({error:'forbidden'},403);const [a,b,c]=await Promise.all([env.DB.prepare('SELECT COUNT(*) n FROM orders').first(),env.DB.prepare("SELECT COALESCE(SUM(total_irt),0) n FROM orders WHERE status IN ('PAID','PROCESSING','SHIPPED','DELIVERED')").first(),env.DB.prepare('SELECT COUNT(*) n FROM users').first()]);return json({orders:a.n,revenue_irt:b.n,users:c.n})}
